...
from echoreason.training.grpo_trainer import GRPOTrainer
...
def main():
    ...
    if fac_cfg.get("enable", False):
        # 支持传目录：目录下默认 labels/ann.json，data_root 指向该目录
    ...
    resume_path = cfg["train_grpo"].get("resume_from", "")
    if resume_path:
        trainer.load_checkpoint(resume_path, map_location=args.device, load_optimizer=True, load_scheduler=True)
    trainer.train()
